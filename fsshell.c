...
struct fs_diriteminfo *fs_readdir(fdDir *dirp)
    ...
    fsDi.d_reclen = (unsigned short) sizeof (fsDi); //I Changed second fsDI from fsDe to FsDi
    fsDi.fileType = de->d_type;
    strcpy(fsDi.d_name, de->d_name);
    return (&fsDi);
    }

int fs_closedir(fdDir *dirp) //I changed fs_DIR to fdDir
    {
    DIR *dir;
    dir = (DIR *) dirp;
    return (closedir (dir));
...
#define BUFFERLEN        512        // Changed from 200
#define DIRMAX_LEN        2048    // Changed from 4096
...
int cmd_cp2l(int argcnt, char *argvec[]) {
    ...
    linux_fd = open(dest, O_WRONLY | O_CREAT | O_TRUNC, 0777);
    do {
...
int main(int argc, char *argv[]) {
    ...
    printf("Opened %s, Volume Size: %llu;  BlockSize: %llu; Return %d\n", filename, (ull_t) volumeSize,
           (ull_t) blockSize, retVal);
    ...
    retVal = initFileSystem(filename, volumeSize / blockSize, blockSize);
